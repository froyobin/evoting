...
var m *sync.Mutex
const (
	NAME       = "9a7b61a46fac930554dab504ff3edb3c194904dc8231ced8c5cd9f88a328ec373ed16c9c3dc018f8669b3cf72a2940e6ee3b5f57b4728cdab2adbeee8bfbe492"
	Base       = 2
	BC   = 32
	PK = 256
	PL = 1360
...
func cast_my_vote(url string, candidate_id, number_can int,
		key *voterskey, Ws *big.Int, randtime int)(bool,RespData) {
	...
	defer func(){
		if err:=recover();err!=nil{
			fmt.Print("error in casting vote")
		}
	}()
	...
	for true {
		...
		if (len(vote.C)) != 0 {
			...
			duration := time.Duration(randtime)
...
func addJob(jobs chan<- int, voter_pool map[int]bool, jmpout bool) {


	for k,v := range voter_pool {
		if v == true{
			continue
		}else {
			fmt.Println("sss")
	...
	fmt.Println("send job done")
	close(jobs)
}
var colorflag = false
var test_flag = true
func doJob(
		jobs <- chan int,voters_pool map[int]bool, dones chan<- struct{},b []byte, url string,
number_can int,
		voter_keys []*voterskey,Ws []*big.Int,timecount *log.Logger,
		ret_event chan *pbchain.Event_ChaincodeEvent, ii int, bar *pb.ProgressBar){

	s1 := mathrand.NewSource(time.Now().Unix())
	r1 := mathrand.New(s1)
	for job := range jobs{
		candidate_id := int(b[job])
		timenow := time.Now()
		randtime := r1.Intn(30)+20
		ret,retData := cast_my_vote(url, candidate_id, number_can,
			voter_keys[job], Ws[job], randtime)
		if ret == false {
			fmt.Println("error in cast the vote, will do it again")
			fmt.Println("return to begin")
			if test_flag {
					test_flag = false
					voters_pool[job] = false
			}
			continue
		}
		leaveloop := false
		...
		timeTrack(timecount, timenow,msg)
		bar.Increment()
	}
	dones <- struct{}{}
...
func main() {
	...
	Get_Ws_N_g("outputWs.txt",Ws, num)

	voter_keys := make([]*voterskey, num, num)

	ret_get_key := get_key_from_file("output.txt", voter_keys, len(voter_keys))
	...
	for ; jmpout==false ;{
		...
		jmpout = true
		addJob(jobs, voters_pool, jmpout)
		fmt.Println("send job done")
		//close(jobs)
		for i := 0; i < worker; i++ {
			go doJob(jobs, voters_pool, dones, b, url, number_can, voter_keys, Ws, timecount, ret_event, i,
				bar)
		}
		for {
			<-dones
	...
	for i:=5;i<6; i++ {
		candidate_id := int(b[i])
		ret,retData := cast_my_vote(url, candidate_id, number_can, voter_keys[i], Ws[i],20)
		if ret == false {
			fmt.Println("error!!")
			return
		}
